...
@CommandType(entity = "LEAD", action = "UPDATE")
@ConditionalOnProperty(value = "fineract.module.lead.enabled", havingValue = "true")
public class UpdateLeadCommandHandler implements NewCommandSourceHandler {

    private final LeadWritePlatformService writePlatformService;

    @Override
    public CommandProcessingResult processCommand(JsonCommand command) {
        return writePlatformService.update(command);
    }
}
